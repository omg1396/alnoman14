...
class AccountInvoice(models.Model):
    ...
    def _compute_amount(self):
        for move in self:
            ...
            print("total_residual_currency",total_residual_currency)
    ...
    #             else:
    #                 move.amount_discount = move.discount_rate
    #                 move.amount_total = sum((line.quantity * line.price_unit) for line in move.invoice_line_ids) - move.amount_discount
    #         else:
    #             move.amount_total = sum((line.quantity * line.price_unit) for line in move.invoice_line_ids)
    #
    #         move.amount_untaxed = sign * (total_untaxed_currency if len(currencies) == 1 else total_untaxed)
    #         move.amount_tax = sign * (total_tax_currency if len(currencies) == 1 else total_tax)
    #         move.amount_residual = -sign * (total_residual_currency if len(currencies) == 1 else total_residual)
