...
class HrLoan(models.Model):
    ...
    payment_type = fields.Selection([('fixed', 'Fixed'),
                                     ('percentage', 'Percentage')], "Installment", default='fixed')
    installment = fields.Integer(string="No Of Installments", default=1, help="Number of installments", tracking=True)
    installment_percentage = fields.Float(string="Installment Percentage ( % ) ", default=1.0)
    ...
    def compute_installment(self):
        ...
        for loan in self:
            ...
            if loan.payment_type == 'fixed':
                ...
                amount = (loan.loan_amount * (loan.installment_percentage / 100))
                no_of_months = math.floor(loan.loan_amount / amount)
                mod_amount = loan.loan_amount % amount
                if mod_amount > 0:
                    no_of_months += 1
                for i in range(1, no_of_months+1):
                    if i == no_of_months and mod_amount > 0:
                        amount = mod_amount
                    self.env['hr.loan.line'].create({
                        'date': date_start,
                        'amount': round(amount),
